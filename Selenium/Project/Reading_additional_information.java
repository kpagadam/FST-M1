...
public class Reading_additional_information {
	WebDriver driver;
	WebDriverWait wait;
	...
	public void readInfo() throws InterruptedException {
		...
		Iterator<WebElement> row = additionalInfo.iterator();
		while (row.hasNext()) {
			row.next().click();
			WebElement leaddetails = form.findElement(By.className("phone"));
			if (leaddetails.getText() != null) {
				System.out.println("Phone :" + leaddetails.getText());
			} else
				continue;
		}
	}
